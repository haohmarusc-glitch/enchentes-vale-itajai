...
- [ ] **Cotas de régua de Gaspar — a maior lacuna que sobrou.** A cidade tem 1.618 cotas de rua e **nenhuma cota de régua**: os números dizem a partir de quanto cada rua alaga, e nada dispara aviso lá. O leitor da tabela municipal está escrito e testado, mas **o host de Gaspar não aceita conexão da VPS** (DNS resolve, IPv4 dá timeout; três tentativas em duas datas — ver `docs/fontes-tempo-real.md`). Dois caminhos: abrir a página no navegador de quem está na região, salvar o HTML e rodar `coleta_gaspar.py --arquivo pagina.html --cotas`; ou, se a tabela só publicar o nível atual, ofício à Defesa Civil de Gaspar pedindo as cotas de atenção/alerta e a régua a que elas se referem.
