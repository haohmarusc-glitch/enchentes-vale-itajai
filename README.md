...
- [ ] **Faixa branda própria, abaixo de "atenção" — a lacuna que o conserto de 04/09 deixou à vista.** O mapa deixou de gritar emergência na fase mais branda de cinco cidades (ver Concluído), e o preço é o outro lado: entre **5,00 e 7,00 m Taió aparece como "normal"** enquanto a COMPDEC de Taió já está **monitorando**; o mesmo vale para `observacao_cota` em Ibirama (3,00–3,50 m) e para o `ativacao_plancon` de Timbó. O `estacoes.json` já previa isso em Taió ("Consequência a saber"). As três saídas, em ordem de honestidade: **(a)** uma faixa nova entre normal e atenção, com nome e cor próprios, que preserva o nome da fase — é a que segue a regra "nome de fase não se rebatiza", e custa legenda + cor + `mapaMotor`; **(b)** mapear para `atencao`, que é justamente o remapeamento de um degrau que o projeto **recusou** ao gravar os dados; **(c)** deixar como está. Não foi decidido aqui porque muda o que o morador vê, e isso é decisão de tela, não conserto de defeito.
