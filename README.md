...
- [ ] **Ponto exato onde o Benedito e o Luís Alves entram no tronco** (antes/depois das réguas de Indaial e Ilhota). **Ferramenta pronta:** `scripts/achar_confluencias.py` resolve por geometria (sem Overpass) — acha a confluência como o ponto de menor toque entre o traçado do afluente e o do tronco, e usa o grafo do tronco (Dijkstra, como `medir_distancia_rio.py`) para dizer entre quais cidades cai, com guarda "não toca" se a geometria estiver incompleta. Falta só rodar **na VPS**, onde estão os GeoJSON dos afluentes (`benedito.geojson`, `luiz-alves.geojson`) — eles não estão no repo. Até lá, `_topologia.afluentes_rios` fica "a confirmar por coordenada" (não se inventa o lado). Travado por `scripts/teste_achar_confluencias.py`.
