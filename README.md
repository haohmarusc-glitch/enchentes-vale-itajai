...
- [ ] **Conciliar o hidrograma de projeto da JICA com a faixa que o texto do estudo afirma.** Investigado em 04/09 e **o diagnóstico anterior estava errado**: ele citava Apiúna, removida do eixo em 02/09. O que existe de fato é uma **sobreposição, não uma contradição**. A Tabela 7.5.1 da JICA, gravada no `_meta` do `transito.json`, dá valor para **todas** as cidades do tronco (`Rio do Sul +0 · Indaial +10 · Blumenau +10 · Gaspar +12 · Ilhota +17 · Itajaí +27`), **é monotônica sozinha**, e os seis trechos gravados batem com ela no **máximo**, um a um — o dado atual *é* o hidrograma, com dois trechos alargados para baixo pelas faixas afirmadas (7–10 h e 14–17 h). A janela de Blumenau começar 3 h antes da de Indaial nasce de comparar o **mínimo de uma fonte** com o **ponto de outra**; testados todos os pares montante/jusante, **nenhum fica sem atribuição consistente** (pior caso: Indaial 10 ≤ Blumenau máx 10). O `validar_dados.py` agora separa os dois casos (erro só quando `min_montante > max_jusante`; sobreposição vira aviso), com quatro testes. **Falta a conciliação de verdade**, que exige o PDF da JICA: conferir se a Tabela 7.5.1 traz mesmo a linha de Indaial. Ver `docs/JANELA-DE-CHEGADA.md`, cuja reestruturação por interpolação foi **recusada** — substituiria valor de fonte por estimativa nossa no horário que o morador lê.
