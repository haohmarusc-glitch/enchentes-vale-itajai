...
- [ ] **Coordenadas das 11 réguas DC de Itajaí → ordenar o Mirim pela descida do rio.** Hoje `estacoes.json` tem `coordenadas: null` nas DC, e a busca no ArcGIS público (02/09) confirmou que a camada de estações da Defesa Civil está **atrás de token** (pasta `defesacivil`); o Plano da COMPDEC traz curso+local mas **nenhuma lat/lon**. Sem coordenada não dá para ordenar as réguas de Itajaí pela descida ao mar (em especial o Mirim, que se divide em canal retificado e curso antigo) — e o projeto não inventa coordenada. Caminhos: complementar o **ofício C2** (GEOItajaí) pedindo as 11 coordenadas, GPS em campo, ou a reunião da Univali (03/09). Detalhe em `docs/coordenadas-dc-itajai.md`.
