...
- [ ] **Rodar `scripts/baixar_itajai_arcgis.py` na VPS.** Traz do ArcGIS público da Prefeitura as três camadas que os documentos citavam mas que nunca chegaram ao repositório: as 10 camadas de inundação, os 5.237 pontos cotados e os 110 polígonos de terreno inundável. Não precisa de navegador. Confere o `robots.txt` do host antes e recusa se não puder lê-lo. **Baixar não destrava a resposta em metros do "meu ponto"** — o problema ali é de referência, não de arquivo ausente.
...
- [ ] **Conseguir as tabelas de cota de rua que faltam.** Rio do Sul e Blumenau saíram (ver Concluído). Brusque e Gaspar saíram também, pelos KML das respectivas Defesas Civis (ver Concluído). Resta **Itajaí** (ArcGIS fechado por token — ofício à prefeitura), que segue sendo a única cidade com manchas de inundação no repositório e nenhuma cota de rua. Hoje são **4.592 pontos**: 2.042 de Blumenau, 1.618 de Gaspar, 555 de Rio do Sul e 377 de Brusque.
...
- [ ] **Trazer `blumenau-enchentes-registradas-alertablu.json` para o repositório.** É a tabela oficial de 102 enchentes de Blumenau (1852–2024), da página `/p/enchentes`. O relato dela chegou; o arquivo, não. Ele resolveria de uma vez o padrão das diferenças contra os nossos 113 registros — e já complicou a regra de referência: o valor que a Defesa Civil publica para set/2011 é **12,60 m**, enquanto o CEOPS publica 13,00 m. São 0,40 m, não os 0,20 m em que a regra bloqueante se apoia. Detalhe em `docs/fontes-academicas.md`.
- [ ] **Régua e cotas de referência de Gaspar.** A cidade tem agora 1.618 cotas de rua, e **nenhuma cota em `estacoes.json`** nem estação de tempo real coletada — então nenhuma delas dispara aviso. Elas respondem "a partir de que nível a minha rua alaga"; falta o "o rio está chegando lá". A fonte provável é `defesacivil.gaspar.sc.gov.br/monitoramento/tabela`, HTML simples. É a maior lacuna de Gaspar hoje, e o validador a repete a cada execução.
