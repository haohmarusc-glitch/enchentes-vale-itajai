...
- [ ] **🟠 MÉDIA — Gráficos chamam leitura antiga de "Agora".** Em `/acu/gaspar`, a série dizia *"Agora: 1,32 m"* medido **18/09 18:03** — 19 h antes — seguido de *"descendo (3 cm/h)"*. O topo da página acerta ("sem leitura ao vivo"); a série não. `LinhaDoTempo.tsx:160` escreve "Agora" incondicionalmente, e a tendência não diz que se refere a um período encerrado.
- [ ] **🟠 MÉDIA — Nível estadual na página da cidade sai sem idade.** Em Brusque, Vidal Ramos e Ilhota o cartão "Agora" mostra o bruto sem o horário da medição; a idade visível logo abaixo é a da **chuva**. No monitor regional o mesmo dado tem idade. `TelaCidade.tsx:185`.
- [ ] **🟠 MÉDIA — Blumenau: o site chama de "duas réguas" o que são duas FONTES.** `LinhaDoTempo.tsx:132` aplica a toda série múltipla o texto de réguas com zeros distintos. Em Blumenau as duas séries são a primária e o AlertaBlu — **a mesma régua**, publicada duas vezes, com mediana de 6,5 cm entre elas. Separar as séries é decisão correta e deliberada; transformar isso em "duas réguas físicas" é afirmação falsa. **O bot já acerta** (junta pelo `resgate_de`): as duas telas se contradizem.
