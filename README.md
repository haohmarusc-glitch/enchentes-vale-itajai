...
- [ ] **Três cursos de Itajaí faltam no mapa — medido, não achado.** Com as onze réguas virando pontos no Monitor, quatro caíam longe de qualquer rio desenhado. `scripts/conferir_reguas_no_tracado.py` (novo) mede a distância de cada uma à **linha** do traçado: **DC-08 Rio do Meio 4,41 km** (Rio Canhanduba), **DC-03 SEMASA 2,32 km** (canal retificado do Mirim), **DC-07 Portal I 2,25 km** e **DC-09 Bairro Murta 0,87 km** (Ribeirão da Murta). As outras sete estão a menos de 0,2 km. **O tronco está certo:** a DC-11, na margem do meandro da **Volta de Cima**, cai a **0,09 km** — o que só acontece se a curva estiver desenhada; a "linha reta" que aparecia num recorte era o meandro **cortado pela moldura do gráfico**. Faltavam porque a consulta original do Overpass pediu só `waterway=river`, e ribeirão é `stream` e canal é `canal`. **O caminho já está pronto dos dois lados:** `converter_tracado_rios.py` lê um bruto SEPARADO (`tracado-ribeiroes-osm.json`, para não tocar no traçado do tronco que já está conferido) e o `MonitorBacia` já lista os três em `AFLUENTES`, que são opcionais — entram sozinhos quando o geojson aparecer. **Falta rodar a consulta na VPS**, que é o único passo que o ambiente de dev não alcança: comando pronto em `docs/tracado-ribeiroes.md`.
...
- [ ] **Vidal Ramos: já tem nível ao vivo, falta a cota.** A régua da API Asthon **está na tela** desde 01/09 (`scripts/coleta_asthon.py`, ver Concluído): nível ao vivo com a idade à vista, faixa cinza. O que resta é a **cota de referência** (atenção/alerta) da régua — sem ela o número aparece e não dispara nada. Taió e Ituporanga, ao contrário, **não** têm régua de cidade na Asthon: têm as barragens Oeste e Sul, nível de reservatório na escala do próprio barramento, que nunca pintam faixa de cidade. Ver `scripts/analisar_asthon.py`. **Caminho novo (04/09):** um levantamento da API relata que o endpoint `panel` traz `band_thresholds` (cota por régua) para 21 estações — o que fecharia esta pendência com o dado na fonte, sem offset nem calibração. **Não confirmado:** a captura que existe no repo traz Vidal Ramos com a lista de cotas VAZIA (só as barragens têm, em escala de reservatório), e a captura do `panel` não veio junto. O comando para capturar e conferir na VPS está em `docs/API-ASTHON-COMPLETA.md` — enquanto não rodar, nada de cota foi gravado.
