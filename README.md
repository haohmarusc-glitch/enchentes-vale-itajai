...
- [ ] **Nível de Gaspar: há cota, falta leitura.** Com as faixas cadastradas, o aviso automático passa a funcionar no instante em que houver um número — e não há. A única fonte é a tabela do próprio município, cujo host dá **timeout de conexão** desde a VPS (`coleta_gaspar.py` só roda por `--arquivo`, com HTML salvo do navegador). A Defesa Civil de SC publica Gaspar, mas só chuva. Caminhos: descobrir por que o host recusa o IP da VPS, ou pedir à Defesa Civil de Gaspar um endpoint estável. Enquanto isso a cota aparece na tela e responde "a partir de que nível a minha rua alaga", mas ninguém é avisado de que o rio está chegando lá.
