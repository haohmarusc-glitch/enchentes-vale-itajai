...
- [ ] **Rodar `scripts/coleta_gaspar.py --cotas` na VPS.** Gaspar tem 1.618 cotas de rua e **nenhuma cota de régua** em `estacoes.json` — os números dizem a partir de quanto cada rua alaga, e nada dispara aviso lá. O leitor da tabela da Defesa Civil de Gaspar está escrito e testado; falta rodar onde o host é alcançável. Ele confere o `robots.txt` antes e **propõe** as faixas, sem gravar. Se a tabela só publicar o nível atual, isso é resposta e não falha: as cotas viriam por ofício à DC Gaspar ou pelo Plano de Contingência, como foi com as onze estações de Itajaí.
