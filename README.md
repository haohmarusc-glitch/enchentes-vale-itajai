...
- [ ] **AlertaBlu continua barrado, e o coletor que chegou pronto não foi adotado.** Uma versão do projeto trouxe `coleta_alertablu.py`, apontando para `defesacivil.blumenau.sc.gov.br`. O site foi recusado antes por `robots.txt` (`docs/cotas-de-ruas.md`) e a sonda de 31/08 não conseguiu nem ler o `robots.txt` de lá. Enquanto isso não for verificado na VPS, o coletor fica fora: código pronto não substitui permissão.
