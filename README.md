...
- [ ] **Conseguir as tabelas de cota de rua que faltam.** Rio do Sul e Blumenau saíram (ver Concluído). Brusque e Gaspar saíram também, pelos KML das respectivas Defesas Civis (ver Concluído). Resta **Itajaí** (ArcGIS fechado por token — ofício à prefeitura), que segue sendo a única cidade com manchas de inundação no repositório e nenhuma cota de rua. Hoje são **4.488 pontos**: 1.938 de Blumenau, 1.636 de Gaspar, 555 de Rio do Sul e 377 de Brusque.
...
- [ ] **Régua e cotas de referência de Gaspar.** A cidade tem agora 1.636 cotas de rua, e **nenhuma cota em `estacoes.json`** nem estação de tempo real coletada — então nenhuma delas dispara aviso. Elas respondem "a partir de que nível a minha rua alaga"; falta o "o rio está chegando lá". A fonte provável é `defesacivil.gaspar.sc.gov.br/monitoramento/tabela`, HTML simples. É a maior lacuna de Gaspar hoje, e o validador a repete a cada execução.
