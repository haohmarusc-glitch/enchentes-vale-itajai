...
`data/cotas-ruas.json` — 2.543 registros de Blumenau, Rio do Sul, Brusque e Gaspar,
cada um com fonte, data e confiança. Regras que o validador e os testes travam:
...
- [ ] **Conseguir as tabelas de cota de rua que faltam.** Rio do Sul e Blumenau saíram (ver Concluído). Restam: **Brusque** (planilha não pública — ofício; o KML do My Maps foi analisado e recusado, ver abaixo), **Gaspar** (mapa "Pesquise sua cota"; o host deu timeout de conexão nas duas sondagens de 31/08/2026, vale repetir) e **Itajaí** (ArcGIS fechado por token — ofício à prefeitura). Hoje são **2.543 pontos**: 1.938 de Blumenau, 555 de Rio do Sul, 27 de Brusque e 23 de Gaspar.
- [ ] **Coletar a API pública Asthon de Rio do Sul.** `public.asthon.com.br`, `city_id 4214805`: 29 estações do Alto Vale com nível ao vivo, barragens e histórico horário — cobre **Taió, Ituporanga e Vidal Ramos**, que hoje não têm nível nenhum na tela. Faixas oficiais da régua da Ponte Dom Tito Buss: 4,50 / 5,50 / 6,50 m, e abrigos a 7,00 m. Antes de coletar, checar `robots.txt` e termos; e só vira aviso se a leitura vier com a cota **daquela** estação.
- [ ] **Baixar o estudo de 2024 da Defesa Civil de Brusque** (`bit.ly/novascotasbrusque`, bloqueia robôs — pelo navegador). São 357 pontos revistos até 8,96 m, e o achado é que as cotas **subiram** em vários bairros por causa dos canais extravasores da Beira-Rio. É a fonte que Brusque precisa, e também o motivo de o levantamento de 2011 estar desatualizado abaixo de ~7,50 m.
...
- [ ] **Google My Maps de Gaspar** (1.615 pontos georreferenciados). Ler antes a recusa do KML de Brusque: mesma origem, mesmo campo `cota`, e lá ele era uma mistura. Baixar com **todos** os campos (`obs`, `esquina`, UTM), que é o que faltou em Brusque.
- [ ] **Pedir o KML original de Brusque, ou a planilha da Defesa Civil.** Chegou ao projeto um KML do Google My Maps com 1.679 pontos de cota, e um conversor que os gravaria como `referencia: "régua", confianca: "alta"`. A análise (`scripts/analisar_kml_brusque.py`) recusou: **64% dos pontos da pasta "Cotas de Cheia 2011" trazem valor acima do pico de 2011** (10,03 m), o maior é 29,53 m, e a mediana sobe com a distância do rio — comportamento de altitude de terreno. Ao mesmo tempo, 4 das 13 ruas em comum batem no centavo com a lista oficial (P por acaso = 0,0001): o arquivo é uma **mistura**, e nenhum campo separa uma coisa da outra. A conversão que recebemos perdeu `obs`, `esquina` e as coordenadas UTM, que é onde estaria a resposta. Brusque fica com 27 pontos. Detalhe em `docs/cotas-de-ruas.md`.
...
- [ ] **Blumenau na fonte de Itajaí está congelada.** Em 31/08/2026, às 20:45 UTC, a página da Defesa Civil de Itajaí publicava Blumenau com carimbo de **197 min** para chuva e **204 min** para nível, enquanto as outras treze estações da mesma página estavam entre 18 e 29 min. Como o site mostra o MAIOR acumulado entre os pluviômetros da cidade, Blumenau passa a exibir os 69,2 mm velhos em vez dos 19,4 mm frescos da Defesa Civil de SC — com a idade correta ao lado, depois do conserto da idade, mas ainda assim o número em destaque é o velho. Decidir se a estação Blumenau da fonte de Itajaí deve ser ignorada; uma observação só não basta para tirar uma fonte.
