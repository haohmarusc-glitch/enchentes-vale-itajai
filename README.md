...
- [ ] **🔴 ALTA — Itajaí perdeu as onze réguas: o portal mudou de endereço.** `coleta_itajai.py:47` fixa `defesacivil.itajai.sc.gov.br/monitoramento/nivel-rios`, que responde **404**. O novo é `monitoramento.defesacivil.itajai.sc.gov.br/monitoramento/rios`, e **não basta trocar o domínio**: o contrato mudou (identificadores `DC01` em vez de `DC-01`, datas com vírgula, separação por `municipio_id`). **Não consigo fazer daqui** — os dois hosts são recusados pela política de saída deste ambiente (`connect_rejected`, conferido uma vez). Para eu escrever o parser preciso do **corpo bruto** da página nova salvo num arquivo. É a cidade da FOZ, com onze réguas e o painel de maré.
...
- [ ] **A fonte municipal de Brusque parou em algum momento de 18/09.** De manhã o pino trazia `1,94 m — Brusque`; à tarde, `1,31 m — SDC-SC Brusque (rede estadual)`. O resgate funcionou e está rotulado, mas **enquanto durar, as 338 cotas de rua de Brusque ficam sem o "faltam X m"** — que é a parte mais útil da resposta. Ver se é queda pontual do portal ou parada da estação, e se o vigia reportou.
