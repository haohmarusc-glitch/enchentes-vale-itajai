...
test('lê a classificação estadual (C7) só quando o coletor a validou; senão null', () => {
  ...
  const normal = montarNivelSc({ leituras: [{ ...base, classificacao_estadual: { faixa: 'normal', ativo: true } }] })
  assert.equal(normal.get('rio-do-sul')?.faixaEstadual, 'normal', 'validado em 14/09: ativo sem flag é normal')
  const lixo = montarNivelSc({ leituras: [{ ...base, classificacao_estadual: { faixa: 'inundacao' } }, { ...base, cidade: 'x', classificacao_estadual: 'atencao' }] })
  assert.equal(lixo.get('rio-do-sul')?.faixaEstadual, null, 'faixa desconhecida e strings soltas não viram faixa')
