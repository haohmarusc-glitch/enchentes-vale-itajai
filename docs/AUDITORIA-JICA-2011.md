...
3. **“7 comportas = 7 condutos”** bate. ~~Não implica que o site da Asthon exponha estado de cada comporta — isso continua lacuna.~~ **LACUNA FECHADA em 05/09/2026:** o endpoint `public.asthon.com.br/public/dams?city_id=4214805` traz `comportas[].aberta` por comporta, mais `montante_m`, `jusante_m`, `percent_use` e `vertido`, para as duas barragens e sem autenticação. Ver `docs/RIO-DO-SUL-COMPORTAS.md`. A **vazão** de saída continua não publicada, como o JICA aponta — mas o número de comportas abertas é o proxy operacional mais próximo.
