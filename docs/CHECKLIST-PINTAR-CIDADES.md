...
- [~] B5 · VPS: `scp` da pasta `historico-dcsc` para `data/series/dcsc-zips/`, consolidador, checagem "IGUAL ao resumo do repo". **A checagem ficou pronta em 15/09** (`scripts/conferir_resumo_dcsc.py` + 10 testes): compara os 13×19 campos contra o resumo commitado, ignora só `_meta.gerado_em` e sai com código 1 em qualquer divergência — a olho seriam 247 comparações, e o erro que importa é o de uma linha. Falta o `scp`, que só o Jefferson pode fazer.
  ⚠️ **A pasta no PC do Jefferson é insubstituível.** A API da DCSC é uma **janela móvel de ~90 dias** (`PROFUNDIDADE_MAX_DIAS = 88`, medido no B6), e aquela cópia, baixada em 10/09, tem série desde out/2022–abr/2023. O `baixar_historico_dcsc.py` **não** a reconstrói. Tratar como original: copiar, não mover.
...
- [x] B9 · C12 enviado à EPAGRI em 10/09/2026 às 21:05 BRT, na thread do C5 (`1a06001cb0e0b6ce`). Gmail ID: `1a08dc8f1ad59701`. **RESPONDIDO em 15/09/2026 08:34 BRT** (Gmail ID `1a0a4d946a23087b`, 8 PDFs): os três Avisos Hidrológicos de nov/2023 e cinco notas hidrometeorológicas. Transcrição e ressalva em `docs/RESPOSTA-EPAGRI-C12-2026-09-15.md`. ⚠️ A fonte declara que aviso é **instantâneo do horário da coleta, não pico** — nada dali entra em `enchentes.json` como pico. Canal aberto para pedidos futuros sem ofício; inscrição nos boletins diários feita.
- [x] B10 · **Medido em 15/09/2026, e a resposta é: são os portais, não a VPS.** Da VPS (Hetzner, Helsinque, 65.108.154.111), as cinco URLs de Brusque e Gaspar dão **timeout de 35 s, código 000** — nenhuma resposta. E não é rota nem a VPS estar quebrada: nos mesmos minutos, do mesmo servidor, `monitoramento.defesacivil.sc.gov.br` devolveu **200 em 0,69 s**, `defesacivil.itajai.sc.gov.br` **200 em 4,35 s** e `www.ana.gov.br` **302 em 0,74 s**. O Brasil é alcançável; estes dois municípios não. Do Windows do Jefferson, no Brasil, os dois abrem: Brusque raiz **200 em 21,6 s**, estação 79 **200 em 2,3 s**, Gaspar **200 em 0,3 s**. Detalhes em `docs/B10-PORTAIS-MUNICIPAIS-2026-09-15.md`.
...
- 16/09/2026 · **LAI Cemaden: não há recurso a interpor.** O "prazo 21/09" que chegou a ser anotado veio da frase automática do Fala.BR (*"No caso de indeferimento..."*), que vai em toda resposta: o pedido 01217.006547/2026-93 pediu a relação de estações com coordenadas e foi **respondido em 10/09** com exatamente isso. Pendência desfeita. O que falta (medições, cotas de acionamento, datum, endpoint das hidrológicas) nunca foi pedido e vira **pedido novo**: rascunho **C24** em `oficios-prontos.md`, sobre `421320321H` (Pomerode, operacional, 860 m da DCSC-00007 suspeita) e `420290901H` (Brusque, inativa, que **não** é a DCSC-00019). Envio: decisão do Jefferson. Segue aberta a **LAI INMET** 21210.009435/2026-61, sem resposta desde 02/09 (prazo ~22/09).
- 17/09/2026 · **Seis PRs**: #295 (AlertaBlu a cada coleta), #301 (vias alagadas de Itajaí), #361 (vírgula nas notas de cota), #362 (vigia não chama documentação de "versão antiga"; pino diz quando a cota está longe demais), #363 (log do relé de Gaspar + **a "inundação histórica" de 8,50 m de Blumenau, que 86% da série supera, saiu de `cotas_m`**) e #364 (registro do primeiro silêncio de Gaspar). Dois achados vieram de **rodar o código contra o cadastro real**, não de revisão: o ponto decimal nas 556 notas e o 8,50 m de Blumenau. Gaspar: estação do município parada em **16/09 17h24**, relé e SSH de pé, cidade em cinza. Caixa de e-mail conferida em 17/09: nada novo além dos boletins diários da EPAGRI; **LAI do INMET muda desde 02/09** (prazo ~22/09).
- 22/09/2026 · **LAI INMET respondida no prazo** (`docs/LAI-INMET-2026-09-22.md`): quatro EMAs na bacia (Indaial em pane), API só com ACT entre instituições, dados brutos sem flag (`9999`/`Null`/branco = sem dado). Não há recurso a interpor; não há token a esperar. Decisão pendente do Jefferson: pedir ACT (recomendação: não) e baixar nov/2008 no BDMEP.
- 22/09/2026 · **Chuva horária do INMET no repositório** (`docs/INMET-CHUVA-2026-09-22.md`): quatro estações, 2006–08/2026, diária e JSON de eventos reproduzidos a partir da horária (0 diferenças); `suspeito` = zeros travados ≥ 20 dias. Nov/2008 em Indaial: 246,6 mm em 4 dias. Nada em dado.
- 22/09/2026 · **HidroWeb do Mirim no repositório** (`docs/HIDROWEB-MIRIM-2026-09-22.md`): 11 estações da ANA, derivados reproduzidos (0 diferenças; picos 32/34). Os recordes de Brusque de 1944/1958/1978 são digitação. 83900000 = régua municipal ao cm (2019–2021). Antecedência a montante ≤ 14 h na maioria. Cinco cheias de Brusque sem registro no cadastro viram decisão do Jefferson. Nada em dado.
- 22/09/2026 · **Cinco cheias de Brusque entraram** (decisão do Jefferson): 1961, 1978, 1983, 2001, 2015, leituras 07h/17h da 83900000, com `pendencia` de crista não medida. 276 registros.
- 22/09/2026 · **Chat local do histórico** (`docs/CHAT-LOCAL.md`) em `/acu` e `/mirim`: sem IA, sem API, só JSONs de `data/`; 15 testes; JSONs do chat baixados só quando a caixa aparece. Rio do Sul mai/2018 → confiança baixa (decisão). 5 testes de navegador novos.
- 23/09/2026 · **Subida em cm/h da cheia de 11–12/09** (`scripts/subida_cheia.py`, seção no doc do evento): Blumenau 62 cm/h às 18h de 11/09, DC-10 95 cm/h; começo da tarde de 11/09 foi a hora mais rápida em quase toda a bacia. Branch `vps/cheia-2026-09-11-12` é repetição: sem PR.
